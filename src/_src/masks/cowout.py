...
def cowout(
    ...
    """Generate a mask for CowOut and CowMix."""

    log_sigma_range = jnp.log(sigma_range[0]), jnp.log(sigma_range[1])
    max_sigma = sigma_range[1]

    def fn(rng: chex.PRNGKey, size: tuple[int, int, int]):
        return cow_masks(size[0], size[1:], log_sigma_range, max_sigma, prop_range, rng)
